...
class _PracticePageState extends State<PracticePage> {
  ...
  final DateTime _workoutStartTime =
      DateTime.now(); // Сделали final и убрали late
  double _totalTonnage = 0.0;

  // Создаем GlobalKey для доступа к таймеру
  final GlobalKey<_WorkoutTimerState> _timerKey =
    ...
    return await showDialog<bool>(
          ...
          builder:
              (context) => AlertDialog(
                ...
                content: Text(
                  'Are you sure you want to finish the workout? All unsaved data will be lost.',
                ),
                actions: [
                  TextButton(
  ...
  Future<bool> _showSaveConfirmation() async {
    final currentDuration =
        _timerKey.currentState?.currentDuration ?? Duration.zero;
              ...
              (context) => AlertDialog(
                ...
                content: Column(
                  ...
                  children: [
                    ...
                    Text(
                      'Total tonnage: ${_totalTonnage.toStringAsFixed(1)} kg',
                    ),
                    SizedBox(height: 10),
                        ...
                        children: [
                          Row(
                            ...
                            children: [
                              Expanded(
                                child: Text(
                                  exerciseRepo.getExerciseName(
                                    programData[index]['exercise'],
                                  ),
                              ...
                              IconButton(
                                ...
                                onPressed: () {
                                  setState(() {
                                    ...
                                    formManager.workoutFields.removeWhere(
                                      (field) => field['exercise'] == idEx,
                                    );
                              ...
                              return Dismissible(
                                ...
                                onDismissed: (direction) {
                                  setState(() {
                                    ...
                                    formManager.workoutFields.removeAt(
                                      globalIndex,
                                    );
                                    formManager.repsControllers[index].removeAt(
                                      setIndex,
                                    );
                                    formManager.weightControllers[index]
                                        .removeAt(setIndex);

                                    formManager.checkedStates[index]?.remove(
                                      setIndex,
                                    );

                                    programData[index]['sets'] =
                                        (programData[index]['sets'] as int) - 1;

                                    if (programData[index]['sets'] == 0) {
                                      programData.removeAt(index);
                                      formManager.repsControllers.removeAt(
                                        index,
                                      );
                                      formManager.weightControllers.removeAt(
                                        index,
                                      );
                                      ...
                                      Expanded(
                                        child: Padding(
                                          ...
                                          child: TextFormField(
                                            controller:
                                                formManager
                                                    .weightControllers[index][setIndex],
                                            onChanged: (value) {
                                              formManager
                                                      .workoutFields[getGlobalIndex(
                                                    index,
                                                    setIndex,
                                                  )]['weight'] =
                                                  value;
                                      ...
                                      Expanded(
                                        child: Padding(
                                          ...
                                          child: TextFormField(
                                            controller:
                                                formManager
                                                    .repsControllers[index][setIndex],
                                            onChanged: (value) {
                                              formManager
                                                      .workoutFields[getGlobalIndex(
                                                    index,
              ...
              child: ElevatedButton.icon(
                onPressed: () async {
                  final result = await showDialog<Map<String, dynamic>>(
                    ...
                    builder:
                        (context) =>
                            AddExerciseDialog(exerciseRepo: exerciseRepo),
                  );
              ...
              child: Container(
                ...
                child: ElevatedButton.icon(
                  ...
                  onPressed: () async {
                    ...
                    if (shouldSave) {
                      final currentDuration =
                          _timerKey.currentState?.currentDuration ??
                          Duration.zero;
                      ...
                      if (tr_id != -1) {
                        ...
                        for (
                          int i = 0;
                          i < formManager.workoutFields.length;
                          i++
                        ) {
                          data.add({
                            'workout': tr_id,
                            'exercise':
                                formManager.workoutFields[i]['exercise'],
                            'set': formManager.workoutFields[i]['set'],
                            'rep': int.parse(
                              formManager.workoutFields[i]['rep'],
                            ),
                            'weight': int.parse(
                              formManager.workoutFields[i]['weight'],
                            ),
                          });
                        }
                        logger.i(data);
        ...
        children: [
          ...
          Column(
            children: [
              ...
              Text(
                '${widget.totalTonnage.toStringAsFixed(1)} kg',
                style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold),
              ),
            ],
          ),
          IconButton(
            icon: Icon(
              widget.isHeaderVisible ? Icons.expand_less : Icons.expand_more,
            ),
...
class _RestTimerDialogState extends State<RestTimerDialog> {
  ...
  Widget build(BuildContext context) {
    return AlertDialog(
      ...
      actions: [
        TextButton(onPressed: _resetTimer, child: Text('Reset')),
        TextButton(
          onPressed:
              _seconds > 0 ? (_isRunning ? _stopTimer : _startTimer) : null,
